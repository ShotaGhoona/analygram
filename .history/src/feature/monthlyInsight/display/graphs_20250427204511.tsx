...
export const NewFollowersGraph = ({ startDate, endDate }: GraphProps) => {
  ...
  const lines = [
    { key: 'followers', name: '新規フォロワー数', stroke: '#8884d8' },
  ];
...
export const ProfileVisitsGraph = ({ startDate, endDate }: GraphProps) => {
  ...
  const lines = [
    { key: 'profile_visits', name: 'プロフィールアクセス', stroke: '#0088fe' },
  ];
...
export const WebsiteTapRateGraph = ({ startDate, endDate }: GraphProps) => {
  ...
  const formattedData = monthlyData.map(item => ({
    date: item.date ? item.date.slice(0, 10) : '',
    website_tap_rate: item.website_tap_rate,
  }));

  const lines = [
    { key: 'website_tap_rate', name: 'ウェブサイトのタップ率', stroke: '#8884d8' },
  ];
