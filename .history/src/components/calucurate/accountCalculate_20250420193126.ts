...
export const aggregateReelsData = (insights: AccountInsight[]) => {
  const monthlyData = insights.reduce<Record<string, any>>((acc, curr) => {
    ...
    acc[yearMonth].reels_reach += curr.reelReach || 0;
    acc[yearMonth].reels_likes += curr.reelLikes || 0;
    acc[yearMonth].reels_comments += curr.reelComments || 0;
    acc[yearMonth].reels_shares += curr.reelShares || 0;
    acc[yearMonth].reels_saves += curr.reelSaves || 0;
...
export const aggregateStoryData = (insights: AccountInsight[]) => {
  const monthlyData = insights.reduce<Record<string, any>>((acc, curr) => {
    ...
    acc[yearMonth].story_likes += curr.storyLikes || 0;
    acc[yearMonth].story_comments += curr.storyComments || 0;
    acc[yearMonth].story_engagement = 
      acc[yearMonth].story_likes +
      acc[yearMonth].story_comments;
