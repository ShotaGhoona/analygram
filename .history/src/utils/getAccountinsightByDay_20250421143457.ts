...
async function getAccountInsightsByDay(igId: string, accessToken: string): Promise<AccountMetrics | null> {
  try {
    ...
    for (const media of mediaList) {
      ...
      metrics[reachKey] = (metrics[reachKey] as number) + (mediaInsights.reach || 0);
      metrics[likesKey] = (metrics[likesKey] as number) + (mediaInsights.likes || 0);
      metrics[commentsKey] = (metrics[commentsKey] as number) + (mediaInsights.comments || 0);
      metrics[savesKey] = (metrics[savesKey] as number) + (mediaInsights.saves || 0);
      metrics[sharesKey] = (metrics[sharesKey] as number) + (mediaInsights.shares || 0);
    }
