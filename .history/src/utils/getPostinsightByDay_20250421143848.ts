    ...
    const { data: { data: mediaList } } = await axios.get(
      `${BASE_URL}/${company.igId}/media`,
      {
        params: {
          ...
          fields: 'id,caption,media_type,media_product_type,media_url,permalink,thumbnail_url,timestamp,username,like_count,comments_count,shares_count,saves_count,reach_count,impressions_count,plays_count,replies_count',
          since: startDate,
          until: endDate,
        },
      }
    ...
    for (const media of mediaList) {
      try {
        ...
        const postData = {
          ...
          shares: media.shares_count ?? 0,
          saves: media.saves_count ?? 0,
          reach: media.reach_count ?? 0,
          impressions: media.impressions_count ?? 0,
          plays: media.plays_count ?? 0,
          replies: media.replies_count ?? 0,
          engagement: (media.like_count ?? 0) + (media.comments_count ?? 0) + (media.shares_count ?? 0) + (media.saves_count ?? 0)
        };
