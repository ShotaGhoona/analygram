...
const DataTable = () => {
  const aggregatedData = useMemo(() => {
    return aggregateMonthlyData(accountInsightData);
  }, []);
        ...
        <tbody>
          {aggregatedData.map((row, index) => (
            <tr key={index} className="border-b hover:bg-gray-50">
              ...
              <td className="px-4 py-2 text-right">{row.reach_total.toLocaleString()}</td>
              <td className="px-4 py-2 text-right">{row.impressions.toLocaleString()}</td>
              <td className="px-4 py-2 text-right">{row.profile_visits.toLocaleString()}</td>
