...
const DataTable = ({ selectedType }: DataTableProps) => {
  ...
  const getAggregatedData = (): Record<string, number | string>[] => {
    ...
    const aggregatedData = insights.reduce((acc, curr) => {
      ...
      acc[yearMonth].reach += curr[`${prefix}Reach`] || 0;
      acc[yearMonth].likes += curr[`${prefix}Likes`] || 0;
      acc[yearMonth].comments += curr[`${prefix}Comments`] || 0;
      acc[yearMonth].shares += curr[`${prefix}Shares`] || 0;
      acc[yearMonth].saves += curr[`${prefix}Saves`] || 0;
