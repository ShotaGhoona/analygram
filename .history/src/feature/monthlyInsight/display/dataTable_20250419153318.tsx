        ...
        <tbody>
          {monthlyData.map((row, index) => (
            <tr key={index} className="border-b hover:bg-gray-50">
              <td className="text-[15px] px-2">{row.date}日</td>
              <td className="text-[15px] px-2 text-right">{row.newFollowers.toLocaleString()}</td>
              <td className="text-[15px] px-2 text-right">{row.impressions.toLocaleString()}</td>
              <td className="text-[15px] px-2 text-right">{row.reach.toLocaleString()}</td>
              <td className="text-[15px] px-2 text-right">{row.profileViews.toLocaleString()}</td>
              <td className="text-[15px] px-2 text-right">{row.websiteTaps.toLocaleString()}</td>
            </tr>
          ))}
        </tbody>
      </table>
