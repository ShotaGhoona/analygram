...
export const NewFollowersGraph = ({ startDate, endDate }: GraphProps) => {
  ...
  return <LineGraph 
    data={monthlyData.map(d => ({ date: d.date, value: d.followers }))} 
    lines={lines} 
    height="100%" 
    title="新規フォロワー数の推移" 
  />;
...
export const ReachImpressionGraph = ({ startDate, endDate }: GraphProps) => {
  ...
  return <LineGraph 
    data={monthlyData.map(d => ({ date: d.date, value: d.reach_total }))} 
    lines={lines} 
    height="100%" 
    title="リーチ・インプレッションの推移" 
  />;
};
...
export const ProfileVisitsGraph = ({ startDate, endDate }: GraphProps) => {
  ...
  return <LineGraph 
    data={monthlyData.map(d => ({ date: d.date, value: d.profile_visits }))} 
    lines={lines} 
    height="100%" 
    title="プロフィールアクセス数の推移" 
  />;
};
...
export const WebsiteTapRateGraph = ({ startDate, endDate }: GraphProps) => {
  ...
  return <LineGraph 
    data={monthlyData.map(d => ({ date: d.date, value: d.website_tap_rate }))} 
    lines={lines} 
    height="100%" 
    title="ウェブサイトのタップ率の推移" 
  />;
}; 