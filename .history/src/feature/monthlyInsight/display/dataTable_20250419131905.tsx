      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b bg-gray-50">
            ...
            <th className="px-4 py-2 text-right font-medium">インプレッション</th>
        ...
        <tbody>
          {monthlyData.map((row, index) => (
            <tr key={index} className="border-b hover:bg-gray-50">
              <td className="px-4 py-2">{row.date}日</td>
              <td className="px-4 py-2 text-right">{row.newFollowers.toLocaleString()}</td>
              ...
              <td className="px-4 py-2 text-right">{row.profileViews.toLocaleString()}</td>
              <td className="px-4 py-2 text-right">{row.websiteTaps.toLocaleString()}</td>
            </tr>
          ))}
        </tbody>
      </table>
