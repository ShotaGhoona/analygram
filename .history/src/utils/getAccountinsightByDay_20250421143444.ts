...
async function getAccountInsightsByDay(igId: string, accessToken: string): Promise<AccountMetrics | null> {
  try {
    ...
    for (const media of mediaList) {
      ...
      metrics[`${typePrefix}Reach` as keyof AccountMetrics] += mediaInsights.reach || 0;
      metrics[`${typePrefix}Likes` as keyof AccountMetrics] += mediaInsights.likes || 0;
      metrics[`${typePrefix}Comments` as keyof AccountMetrics] += mediaInsights.comments || 0;
      metrics[`${typePrefix}Saves` as keyof AccountMetrics] += mediaInsights.saves || 0;
      metrics[`${typePrefix}Shares` as keyof AccountMetrics] += mediaInsights.shares || 0;
    }
