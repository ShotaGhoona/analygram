    ...
    <div className="overflow-x-auto">
      <table className="min-w-full divide-y divide-gray-200">
        <tbody className="bg-white divide-y divide-gray-200">
          ...
          <tr>
            <th className="w-[150px] px-1 text-center text-xs font-medium text-gray-500 uppercase tracking-wider bg-gray-50">いいね</th>
            {data.map((post) => (<td key={post.id} className="px-6 text-center whitespace-nowrap text-sm text-gray-900">{post.likes}</td>))}
          </tr>
          <tr>
            <th className="w-[150px] px-1 text-center text-xs font-medium text-gray-500 uppercase tracking-wider bg-gray-50">コメント</th>
            {data.map((post) => (<td key={post.id} className="px-6 text-center whitespace-nowrap text-sm text-gray-900">{post.comments}</td>))}
          </tr>
          <tr>
            <th className="w-[150px] px-1 text-center text-xs font-medium text-gray-500 uppercase tracking-wider bg-gray-50">保存</th>
            {data.map((post) => (<td key={post.id} className="px-6 text-center whitespace-nowrap text-sm text-gray-900">{post.saved}</td>))}
          </tr>
          <tr>
            <th className="w-[150px] px-1 text-center text-xs font-medium text-gray-500 uppercase tracking-wider bg-gray-50">シェア</th>
          ...
          <tr>
            ...
            {data.map((post) => (<td key={post.id} className="px-6 text-center py-4 whitespace-nowrap text-sm text-gray-900">{post.mediaProductType}</td>))}
          </tr>
          <tr>
            ...
            {data.map((post) => (<td key={post.id} className="px-6 text-center py-4 whitespace-nowrap text-sm text-gray-900">{post.impressions}</td>))}
          </tr>
          {/* Reels専用の再生回数 */}
          <tr>
            <th className="w-[150px] px-1 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider bg-gray-50">再生回数</th>
            {data.map((post) => (<td key={post.id} className="px-6 text-center py-4 whitespace-nowrap text-sm text-gray-900">{post.plays || '-'}</td>))}
          </tr>
          {/* Story専用のリプライ数 */}
          <tr>
            <th className="w-[150px] px-1 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider bg-gray-50">リプライ</th>
            {data.map((post) => (<td key={post.id} className="px-6 text-center py-4 whitespace-nowrap text-sm text-gray-900">{post.replies || '-'}</td>))}
          </tr>
        </tbody>
      </table>
    </div>
